		...
		if (root != null && root.has("policyInfo")) {
			...
			if (policyInfo != null && policyInfo.isArray()) {
				...
				if (decision.equals("Permit")) {
					...
					boolean isDeleted = proposalDAO.deleteProposal(proposal,
					...
					if (isDeleted) {
						...
						NotifyAllExistingInvestigators(proposalId,
								projectTitle, proposal, notificationMessage,
								"Proposal", true);
						...
						return Response.status(403)
								.entity("NOT able to Delete").build();
					}
				} else {
					return Response.status(403)
							.type(MediaType.APPLICATION_JSON)
					...
					if (root != null && root.has("proposalId")) {
						...
						if (proposalId != null) {
							...
							if (!proposalID.equals("0")
								...
								// For Proposal User Title : for Research
								// Administrator and
								// University Research Director
								if (root != null
										&& root.has("proposalUserTitle")) {
											...
											if (buttonType != null) {
												...
												if (root != null
													...
													if (commonObj != null
														...
														userPositionType = commonObj
																.get("UserPositionType")
																.textValue();
													}
													if (commonObj != null
															&& commonObj
																	.has("UserPositionTitle")) {
														userPositionTitle = commonObj
																.get("UserPositionTitle")
																.textValue();
													}
												}
				...
				if (decision.equals("Permit")) {
					...
					if (root != null && root.has("proposalInfo")) {
						...
						if (proposalInfo != null
								&& proposalInfo.has("ProposalID")) {
							proposalID = proposalInfo.get("ProposalID")
									.textValue();
							if (!proposalID.equals("0")) {
						...
						if (proposalInfo != null
								&& proposalInfo.has("ProjectInfo")) {
							JsonNode projectInfo = proposalInfo
									.get("ProjectInfo");
							if (projectInfo != null
									&& projectInfo.has("ProjectTitle")) {
								if (!proposalID.equals("0")) {
									if (!existingProposal
											.getProjectInfo()
											.getProjectTitle()
											.equals(projectInfo.get(
									...
									newProjectInfo.setProjectTitle(projectInfo
											.get("ProjectTitle").textValue());
								}
							}

							if (projectInfo != null
									&& projectInfo.has("ProjectType")) {
								ProjectType projectType = new ProjectType();
								switch (projectInfo.get("ProjectType")
										.textValue()) {
								case "1":
							...
							if (projectInfo != null
									&& projectInfo.has("TypeOfRequest")) {
								TypeOfRequest typeOfRequest = new TypeOfRequest();
								switch (projectInfo.get("TypeOfRequest")
										.textValue()) {
								case "1":
							...
							if (!proposalID.equals("0")) {
								if (!existingProposal.getProjectInfo()
									...
									existingProposal.getProjectInfo()
											.setProjectPeriod(projectPeriod);
								}
							} else {
						...
						// SponsorAndBudgetInfo
						if (!proposalID.equals("0")) {
							if (!existingProposal.getSponsorAndBudgetInfo()
									.equals(newSponsorAndBudgetInfo)) {
								existingProposal
						...
						if (proposalInfo != null
								&& proposalInfo.has("CostShareInfo")) {
							JsonNode costShareInfo = proposalInfo
									.get("CostShareInfo");
							if (costShareInfo != null
									&& costShareInfo
											.has("InstitutionalCommitted")) {
								switch (costShareInfo.get(
										"InstitutionalCommitted").textValue()) {
								case "1":
									newCostShareInfo
											.setInstitutionalCommitted(true);
									break;
								case "2":
									newCostShareInfo
											.setInstitutionalCommitted(false);
									break;
								default:
									break;
								}
							...
							if (costShareInfo != null
								...
								switch (costShareInfo
										.get("ThirdPartyCommitted").textValue()) {
								case "1":
									newCostShareInfo
											.setThirdPartyCommitted(true);
									break;
								case "2":
									newCostShareInfo
											.setThirdPartyCommitted(false);
									break;
								default:
									break;
								}
						...
						if (!proposalID.equals("0")) {
							if (!existingProposal.getCostShareInfo().equals(
									newCostShareInfo)) {
								existingProposal
										.setCostShareInfo(newCostShareInfo);
							}
						} else {
						...
						if (proposalInfo != null
								&& proposalInfo.has("UnivCommitments")) {
							...
							if (univCommitments != null
									&& univCommitments
											.has("RentalSpaceRequired")) {
								switch (univCommitments.get(
										"RentalSpaceRequired").textValue()) {
								case "1":
									newUnivCommitments
											.setRentalSpaceRequired(true);
									break;
								case "2":
									newUnivCommitments
											.setRentalSpaceRequired(false);
									break;
								default:
									break;
								}
						...
						// UnivCommitments
						if (!proposalID.equals("0")) {
							if (!existingProposal.getUniversityCommitments()
									.equals(newUnivCommitments)) {
								existingProposal
						...
						if (proposalInfo != null
							...
							if (conflicOfInterestInfo != null
									&& conflicOfInterestInfo
											.has("ConflictDisclosed")) {
								switch (conflicOfInterestInfo.get(
										"ConflictDisclosed").textValue()) {
								case "1":
									newConflictOfInterest
											.setConflictDisclosed(true);
									break;
								case "2":
									newConflictOfInterest
						...
						// ConflicOfInterestInfo
						if (!proposalID.equals("0")) {
							if (!existingProposal.getConflicOfInterest()
									.equals(newConflictOfInterest)) {
								existingProposal
						...
						Boolean irbApprovalRequired = false;
						if (proposalInfo != null
								&& proposalInfo.has("ComplianceInfo")) {
							JsonNode complianceInfo = proposalInfo
									.get("ComplianceInfo");
							if (complianceInfo != null
									&& complianceInfo
											.has("InvolveUseOfHumanSubjects")) {
								switch (complianceInfo.get(
										"InvolveUseOfHumanSubjects")
										.textValue()) {
								case "1":
									newComplianceInfo
									...
									if (complianceInfo != null
										...
										switch (complianceInfo
												.get("IRBPending").textValue()) {
										case "1":
											newComplianceInfo
													.setIrbPending(false);
											if (complianceInfo != null
								...
								case "1":
									...
									if (complianceInfo != null
											&& complianceInfo
													.has("IACUCPending")) {
										switch (complianceInfo.get(
												"IACUCPending").textValue()) {
										case "1":
											newComplianceInfo
													.setIacucPending(false);
											if (complianceInfo != null
													&& complianceInfo
															.has("IACUC")) {
												newComplianceInfo
							...
							if (complianceInfo != null
									&& complianceInfo
											.has("InvolveBiosafetyConcerns")) {
								switch (complianceInfo.get(
										"InvolveBiosafetyConcerns").textValue()) {
								case "1":
									newComplianceInfo
											.setInvolveBiosafetyConcerns(true);
									irbApprovalRequired = true;
									if (complianceInfo != null
											&& complianceInfo.has("IBCPending")) {
										switch (complianceInfo
												.get("IBCPending").textValue()) {
										case "1":
											newComplianceInfo
													.setIbcPending(false);
											if (complianceInfo != null
							...
							if (complianceInfo != null
								...
								switch (complianceInfo
										.get("InvolveEnvironmentalHealthAndSafetyConcerns")
										.textValue()) {
								case "1":
									newComplianceInfo
						...
						if (!proposalID.equals("0")) {
							if (!existingProposal.getComplianceInfo().equals(
									newComplianceInfo)) {
								existingProposal
										.setComplianceInfo(newComplianceInfo);
								existingProposal
						...
						if (proposalInfo != null
								&& proposalInfo.has("AdditionalInfo")) {
							...
							if (additionalInfo != null
								...
								switch (additionalInfo
										.get("RelatedToCenterForAdvancedEnergyStudies")
										.textValue()) {
								case "1":
									newAdditionalInfo
						...
						if (proposalInfo != null
							...
							if (collaborationInfo != null
									&& collaborationInfo
											.has("InvolveNonFundedCollab")) {
								switch (collaborationInfo.get(
										"InvolveNonFundedCollab").textValue()) {
								case "1":
						...
						// CollaborationInfo
						if (!proposalID.equals("0")) {
							if (!existingProposal.getCollaborationInfo()
									.equals(newCollaborationInfo)) {
								existingProposal
								...
								case "1":
									...
									if (confidentialInfo != null
											&& confidentialInfo
													.has("Patentable")) {
										newConfidentialInfo
												.setPatentable(confidentialInfo
														.get("Patentable")
														.booleanValue());
									}
									if (confidentialInfo != null
											&& confidentialInfo
													.has("Copyrightable")) {
										newConfidentialInfo
												.setCopyrightable(confidentialInfo
														.get("Copyrightable")
														.booleanValue());
									}
									break;
						...
						if (proposalInfo != null
								&& proposalInfo.has("OSPSectionInfo")) {
							...
							if (oSPSectionInfo != null
								...
								if (!proposalID.equals("0")) {
									if (!existingProposal
											.getOspSectionInfo()
											.getListAgency()
											.equals(oSPSectionInfo.get(
													"ListAgency").textValue())) {
							...
							if (oSPSectionInfo != null
								...
								newFundingSource
										.setFederalFlowThrough(oSPSectionInfo
												.get("FederalFlowThrough")
												.booleanValue());
							}
							...
							if (oSPSectionInfo != null
								...
								newFundingSource
										.setStateOfIdahoEntity(oSPSectionInfo
												.get("StateOfIdahoEntity")
												.booleanValue());
							}
							...
							if (oSPSectionInfo != null
								...
								newFundingSource
										.setPrivateForProfit(oSPSectionInfo
												.get("PrivateForProfit")
												.booleanValue());
							}

							if (oSPSectionInfo != null
									&& oSPSectionInfo
											.has("NonProfitOrganization")) {
								newFundingSource
										.setNonProfitOrganization(oSPSectionInfo
												.get("NonProfitOrganization")
												.booleanValue());
							}

							if (oSPSectionInfo != null
									&& oSPSectionInfo
											.has("NonIdahoStateEntity")) {
								newFundingSource
										.setNonIdahoStateEntity(oSPSectionInfo
												.get("NonIdahoStateEntity")
												.booleanValue());
							}

							if (oSPSectionInfo != null
									&& oSPSectionInfo
											.has("CollegeOrUniversity")) {
								newFundingSource
										.setCollegeOrUniversity(oSPSectionInfo
												.get("CollegeOrUniversity")
												.booleanValue());
							}

							if (oSPSectionInfo != null
									&& oSPSectionInfo.has("LocalEntity")) {
								newFundingSource.setLocalEntity(oSPSectionInfo
										.get("LocalEntity").booleanValue());
							}

							if (oSPSectionInfo != null
									&& oSPSectionInfo
											.has("NonIdahoLocalEntity")) {
								newFundingSource
										.setNonIdahoLocalEntity(oSPSectionInfo
												.get("NonIdahoLocalEntity")
												.booleanValue());
							}
							...
							if (oSPSectionInfo != null
								...
								newFundingSource
										.setTirbalGovernment(oSPSectionInfo
												.get("TirbalGovernment")
												.booleanValue());
							}

							if (oSPSectionInfo != null
									&& oSPSectionInfo.has("Foreign")) {
							...
							// CFDA No
							if (oSPSectionInfo != null
									&& oSPSectionInfo.has("CFDANo")) {
								if (!proposalID.equals("0")) {
									if (!existingProposal
											.getOspSectionInfo()
											.getCfdaNo()
											.equals(oSPSectionInfo
													.get("CFDANo").textValue())) {
										existingProposal.getOspSectionInfo()
												.setCfdaNo(
														oSPSectionInfo.get(
																"CFDANo")
							...
							if (oSPSectionInfo != null
									&& oSPSectionInfo.has("TDC")) {
								newBaseInfo.setTdc(oSPSectionInfo.get("TDC")
										.booleanValue());
							}

							if (oSPSectionInfo != null
									&& oSPSectionInfo.has("TC")) {
							...
							if (oSPSectionInfo != null
								...
								newBaseInfo.setOther(oSPSectionInfo
										.get("Other").booleanValue());
							}

							if (oSPSectionInfo != null
									&& oSPSectionInfo.has("NotApplicable")) {
								newBaseInfo.setNotApplicable(oSPSectionInfo
										.get("NotApplicable").booleanValue());
							}
							...
							if (oSPSectionInfo != null
								...
								switch (oSPSectionInfo
										.get("IsPISalaryIncluded").textValue()) {
								case "1":
									newOSPSectionInfo.setPiSalaryIncluded(true);
									break;
								case "2":
									newOSPSectionInfo
											.setPiSalaryIncluded(false);
									break;
								default:
									break;
								}
							...
							if (oSPSectionInfo != null
									&& oSPSectionInfo
											.has("ThirdPartyCostDocumented")) {
								switch (oSPSectionInfo.get(
										"ThirdPartyCostDocumented").textValue()) {
								case "1":
									newBaseOptions.setYes(true);
									break;
								case "2":
							...
							if (oSPSectionInfo != null
									&& oSPSectionInfo
											.has("IsAnticipatedSubRecipients")) {
								switch (oSPSectionInfo.get(
										"IsAnticipatedSubRecipients")
										.textValue()) {
								case "1":
									newOSPSectionInfo
											.setAnticipatedSubRecipients(true);
									if (oSPSectionInfo != null
											&& oSPSectionInfo
							...
							if (oSPSectionInfo != null
									&& oSPSectionInfo
											.has("PIEligibilityWaiver")) {
								switch (oSPSectionInfo.get(
										"PIEligibilityWaiver").textValue()) {
								case "1":
							...
							if (oSPSectionInfo != null
									&& oSPSectionInfo
											.has("ConflictOfInterestForms")) {
								switch (oSPSectionInfo.get(
										"ConflictOfInterestForms").textValue()) {
								case "1":
									newBaseOptions.setYes(true);
									break;
								case "2":
							...
							if (oSPSectionInfo != null
									&& oSPSectionInfo
											.has("ExcludedPartyListChecked")) {
								switch (oSPSectionInfo.get(
										"ExcludedPartyListChecked").textValue()) {
								case "1":
									newBaseOptions.setYes(true);
									break;
								case "2":
						...
						if (proposalInfo != null
								&& proposalInfo.has("SignatureInfo")) {
							...
							if (!proposalID.equals("0")) {
								if (!existingProposal.getSignatureInfo()
										.equals(allSignatureInfo)) {
						...
						if (proposalInfo != null
							...
							String[] rows = proposalInfo
									.get("InvestigatorInfo").textValue()
									.split("#!#");
							...
							for (String col : rows) {
								...
								investigatorRefAndPosition
										.setUserProfileId(cols[1]);
								investigatorRefAndPosition.setCollege(cols[2]);
								investigatorRefAndPosition
										.setDepartment(cols[3]);
								investigatorRefAndPosition
										.setPositionType(cols[4]);
								investigatorRefAndPosition
										.setPositionTitle(cols[5]);
								switch (cols[0]) {
								case "0":
									if (!proposalID.equals("0")) {
										// if
							...
							if (proposalID.equals("0")) {
								...
								for (InvestigatorRefAndPosition coPI : existingInvestigators
									...
									if (!existingProposal.getInvestigatorInfo()
										...
										deletedInvestigators.getCo_pi().add(
												coPI);
									}
								}
						...
						if (proposalUserTitle != null) {
							...
							if (!proposalRoles.asText().equals("")) {
								currentProposalRoles = Arrays
										.asList(proposalRoles.textValue()
												.split(", "));
							}
							...
							if (buttonType != null) {
								...
								case "Save":
									// This is Hack
									// TODO check for if no Co-PI/ Senior
									// Personnel in first
									// create
									// case
									// Change status to ready to submitted by PI
									if (proposalID.equals("0")) {
										if (newProposal.getInvestigatorInfo()
												.getCo_pi().size() == 0) {
											newProposal
								...
								case "Approve":
									if (!proposalID.equals("0")
										...
										List<SignatureInfo> signatures = proposalDAO
												.findSignaturesExceptInvestigator(
										...
										if (existingProposal.getChairApproval() == ApprovalType.READYFORAPPROVAL
											...
											if (signedByAllIRBs) {
												...
												if (existingProposal
														.getDeanApproval() == ApprovalType.APPROVED
														&& existingProposal
						...
						if (root != null && root.has("gpmsCommonObj")) {
							JsonNode commonObj = root.get("gpmsCommonObj");
							if (commonObj != null
									&& commonObj.has("UserProfileID")) {
								userProfileID = commonObj.get("UserProfileID")
										.textValue();
							}
							if (commonObj != null && commonObj.has("UserName")) {
								userName = commonObj.get("UserName")
										.textValue();
							}
							if (commonObj != null
									&& commonObj.has("UserIsAdmin")) {
								userIsAdmin = Boolean.parseBoolean(commonObj
										.get("UserIsAdmin").textValue());
							}
							if (commonObj != null
							...
							if (commonObj != null
									&& commonObj.has("UserDepartment")) {
								userDepartment = commonObj
										.get("UserDepartment").textValue();
							}
							if (commonObj != null
									&& commonObj.has("UserPositionType")) {
								userPositionType = commonObj.get(
										"UserPositionType").textValue();
							}
							if (commonObj != null
									&& commonObj.has("UserPositionTitle")) {
								userPositionTitle = commonObj.get(
						...
						if (!proposalID.equals("0")) {
							...
							if (!existingProposal.equals(oldProposal)) {
								...
								// need to check the duplicate notification
								// cause there
								// going to be update as well as added as PI,
								// Co-PI,
								// Senior?
								notificationMessage = "Updated by "
										+ authorUserName + ".";
								NotifyAllExistingInvestigators(existingProposal
										.getId().toString(), existingProposal
							...
							proposalDAO
									.saveProposal(newProposal, authorProfile);
							...
							NotifyAllExistingInvestigators(newProposal.getId()
									.toString(), newProposal.getProjectInfo()
									.getProjectTitle(), newProposal,
