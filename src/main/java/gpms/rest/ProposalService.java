		...
		if (root != null && root.has("proposalInfo")) {
					...
					case "Create":
						...
						if (proposalID.equals("0")) {
							if ((newProposal.getInvestigatorInfo().getCo_pi().size() == 0)
									&& (newProposal.getInvestigatorInfo().getSeniorPersonnel().size() == 0)) {
								newProposal.setReadyForSubmissionByPI(ReadyType.READYFORSUBMIT);
					...
					case "Submit":
						...
						if (!proposalID.equals("0") && currentProposalRoles != null) {
							if (existingProposal.getSubmittedByPI() == SubmitType.NOTSUBMITTED
									&& existingProposal.getReadyForSubmissionByPI() == ReadyType.READYFORSUBMIT
								...
								if (seniorCount > 0) {
									for (InvestigatorRefAndPosition senior : existingProposal.getInvestigatorInfo()
											.getSeniorPersonnel()) {
										for (SignatureInfo sign : existingProposal.getSignatureInfo()) {
											if (senior.getUserProfileId().equals(sign.getUserProfileId())
													&& sign.getPositionTitle().equals("Senior")) {
												foundSenior = true;
												break;
											}
										}
								...
								if (signedByPI && signedByAllCoPIs && signedByAllSeniors) {
									...
									existingProposal.setReadyForSubmissionByPI(ReadyType.NOTREADYFORSUBMIT);
					...
					case "Update":
						if (!proposalID.equals("0") && currentProposalRoles != null) {
							if ((currentProposalRoles.contains("PI")
									|| (currentProposalRoles.contains(
											"CO-PI")
									&& existingProposal.getReadyForSubmissionByPI() == ReadyType.NOTREADYFORSUBMIT)
									|| (currentProposalRoles.contains("Senior") && existingProposal
								...
								if (signedByPI && signedByAllCoPIs && signedByAllSeniors) {
									existingProposal.setReadyForSubmissionByPI(ReadyType.READYFORSUBMIT);
			...
			if (addedSignatures.size() != 0) {
				for (SignatureInfo signatureInfo : addedSignatures) {
					...
					for (InvestigatorRefAndPosition senior : currentProposal.getInvestigatorInfo()
							.getSeniorPersonnel()) {
						notification = new NotificationLog();
						notification.setType("Signature");
						notification.setAction("Signed by " + signFullName + " as " + position + ".");
