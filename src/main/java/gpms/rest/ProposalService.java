...
public class ProposalService {
	...
	public String exportProposalsJSON(String message)
		...
		if (proposals.size() > 0) {
			...
			String policyLocation = this.getClass().getResource("/tmpfiles").toURI().getPath();
	...
	public List<SignatureInfo> getAllSignatureForAProposal(String message)
		...
		List<SignatureInfo> signatures = proposalDAO.findAllSignatureForAProposal(id);
						...
						if (proposalUserTitle != null) {
							...
							if (root != null && root.has("buttonType")) {
								...
								if (buttonType != null) {
									...
									case "Withdraw":
										if (!proposalID.equals("0")) {
											if (existingProposal.getProposalStatus().contains(Status.APPROVEDBYDEAN)
													&& proposalUserTitle.textValue().equals("Research Administrator")
													&& !existingProposal.getProposalStatus()
															.contains(Status.WITHDRAWBYRESEARCHADMIN)) {
									...
									case "Archive":
										if (!proposalID.equals("0")) {
											if (existingProposal.getProposalStatus()
													.contains(Status.SUBMITTEDBYRESEARCHADMIN)
													&& proposalUserTitle.textValue()
															.equals("University Research Director")
													&& !existingProposal.getProposalStatus()
															.contains(Status.ARCHIVEDBYRESEARCHDIRECTOR)) {
												existingProposal.getProposalStatus().clear();
												existingProposal.getProposalStatus()
									...
									if (root != null && root.has("gpmsCommonObj")) {
										...
										if (commonObj != null && commonObj.has("UserIsAdmin")) {
											userIsAdmin = commonObj.get("UserIsAdmin").booleanValue();
										}
										if (commonObj != null && commonObj.has("UserCollege")) {
											userCollege = commonObj.get("UserCollege").textValue();
										}
	...
	public Response saveUpdateProposal(String message) throws Exception {
			...
			if (proposalInfo != null && proposalInfo.has("ProposalID")) {
				...
				if (proposalUserTitle != null) {
					...
					if (proposalRoles != null) {
						// TODO
						List<String> currentProposalRoles = Arrays.asList(proposalRoles.textValue().split(", "));
						...
						if (buttonType != null) {
							...
							case "Submit":
								...
								if (!proposalID.equals("0")) {
									...
									List<Status> canPISubmitStatus = Arrays.asList(Status.NOTSUBMITTEDBYPI,
											Status.RETURNEDBYCHAIR, Status.DISAPPROVEDBYBUSINESSMANAGER,
											Status.DISAPPROVEDBYIRB, Status.RETURNEDBYDEAN,
											Status.DISAPPROVEDBYRESEARCHADMIN, Status.DISAPPROVEDBYRESEARCHDIRECTOR);
									if (canPISubmitStatus.contains(existingProposal.getProposalStatus().get(0))
											&& currentProposalRoles.contains("PI")
											&& !proposalUserTitle.textValue().equals("Research Administrator")
											&& !existingProposal.getProposalStatus()
													.contains(Status.WAITINGFORCHAIRAPPROVAL)) {
										existingProposal.getProposalStatus().clear();
										existingProposal.getProposalStatus().add(Status.WAITINGFORCHAIRAPPROVAL);
									} else if (existingProposal.getProposalStatus().contains(Status.READYFORSUBMISSION)
											&& !currentProposalRoles.contains("PI")
											&& proposalUserTitle.textValue().equals("Research Administrator")
											&& !existingProposal.getProposalStatus()
													.contains(Status.SUBMITTEDBYRESEARCHADMIN)) {
									...
									newProposal.getProposalStatus().add(Status.WAITINGFORCHAIRAPPROVAL);
								}
							...
							case "Approve":
								if (!proposalID.equals("0")) {
									...
									// "University Research Director" ];
									if (existingProposal.getProposalStatus().contains(Status.WAITINGFORCHAIRAPPROVAL)
											&& proposalUserTitle.textValue().equals("Department Chair")
											&& !existingProposal.getProposalStatus().contains(Status.READYFORREVIEW)) {
										existingProposal.getProposalStatus().clear();
										// Ready for Review
										existingProposal.getProposalStatus().add(Status.READYFORREVIEW);

									} else if ((existingProposal.getProposalStatus().contains(Status.READYFORREVIEW)
											|| existingProposal.getProposalStatus().contains(Status.APPROVEDBYIRB))
											&& proposalUserTitle.textValue().equals("Business Manager")
											&& !existingProposal.getProposalStatus()
													.contains(Status.REVIEWEDBYBUSINESSMANAGER)) {
										existingProposal.getProposalStatus().clear();
										// Reviewed by Business Manager
										existingProposal.getProposalStatus().add(Status.REVIEWEDBYBUSINESSMANAGER);
										...
										existingProposal.getProposalStatus().clear();
										// Approved by IRB
										existingProposal.getProposalStatus().add(Status.APPROVEDBYIRB);
											...
											&& !existingProposal.getProposalStatus().contains(Status.APPROVEDBYDEAN)) {
										existingProposal.getProposalStatus().clear();
										// Approved by Dean
										existingProposal.getProposalStatus().add(Status.APPROVEDBYDEAN);

									} else if (existingProposal.getProposalStatus().contains(Status.APPROVEDBYDEAN)
											&& proposalUserTitle.textValue().equals("Research Administrator")
											&& !existingProposal.getProposalStatus()
													.contains(Status.SUBMITTEDTORESEARCHDIRECTOR)) {
										existingProposal.getProposalStatus().clear();
										// Submitted to Research Director
										existingProposal.getProposalStatus().add(Status.SUBMITTEDTORESEARCHDIRECTOR);
							...
							case "Disapprove":
								if (!proposalID.equals("0")) {
									...
									if (existingProposal.getProposalStatus().contains(Status.WAITINGFORCHAIRAPPROVAL)
											&& proposalUserTitle.textValue().equals("Department Chair")
											&& !existingProposal.getProposalStatus().contains(Status.RETURNEDBYCHAIR)) {
										existingProposal.getProposalStatus().clear();
										// Returned by Chair
										existingProposal.getProposalStatus().add(Status.RETURNEDBYCHAIR);
											...
											&& proposalUserTitle.textValue().equals("Business Manager")
											&& !existingProposal.getProposalStatus()
													.contains(Status.DISAPPROVEDBYBUSINESSMANAGER)) {
										existingProposal.getProposalStatus().clear();
										...
										existingProposal.getProposalStatus().add(Status.DISAPPROVEDBYBUSINESSMANAGER);
											...
											|| existingProposal.getProposalStatus()
													.contains(Status.REVIEWEDBYBUSINESSMANAGER))
											&& proposalUserTitle.textValue().equals("IRB") && !existingProposal
													.getProposalStatus().contains(Status.DISAPPROVEDBYIRB)) {
										existingProposal.getProposalStatus().clear();
										// Disapproved by IRB
										existingProposal.getProposalStatus().add(Status.DISAPPROVEDBYIRB);
											...
											|| existingProposal.getProposalStatus().contains(Status.APPROVEDBYIRB))
											&& proposalUserTitle.textValue().equals("Dean")
											&& !existingProposal.getProposalStatus().contains(Status.RETURNEDBYDEAN)) {
										existingProposal.getProposalStatus().clear();
										// Returned by Dean
										existingProposal.getProposalStatus().add(Status.RETURNEDBYDEAN);
											...
											&& proposalUserTitle.textValue().equals("Research Administrator")
											&& !existingProposal.getProposalStatus()
													.contains(Status.DISAPPROVEDBYRESEARCHADMIN)) {
										existingProposal.getProposalStatus().clear();
										...
										existingProposal.getProposalStatus().add(Status.DISAPPROVEDBYRESEARCHADMIN);
											...
											&& proposalUserTitle.textValue().equals("University Research Director")
											&& !existingProposal.getProposalStatus()
													.contains(Status.DISAPPROVEDBYRESEARCHDIRECTOR)) {
										existingProposal.getProposalStatus().clear();
			...
			if (projectInfo != null && projectInfo.has("ProjectTitle")) {
				if (!proposalID.equals("0")) {
					if (!existingProposal.getProjectInfo().getProjectTitle()
						...
						existingProposal.getProjectInfo().setProjectTitle(projectInfo.get("ProjectTitle").textValue());
					}
				} else {
		...
		if (proposalInfo != null && proposalInfo.has("SponsorAndBudgetInfo")) {
			...
			if (sponsorAndBudgetInfo != null && sponsorAndBudgetInfo.has("FACosts")) {
				newSponsorAndBudgetInfo.setFACosts(Double.parseDouble(sponsorAndBudgetInfo.get("FACosts").textValue()));
			}
			...
			if (sponsorAndBudgetInfo != null && sponsorAndBudgetInfo.has("FARate")) {
				newSponsorAndBudgetInfo.setFARate(Double.parseDouble(sponsorAndBudgetInfo.get("FARate").textValue()));
			}
		}
		...
		if (proposalInfo != null && proposalInfo.has("InvestigatorInfo")) {
			...
			for (String col : rows) {
				...
				case "0":
					if (!proposalID.equals("0")) {
						if (!existingProposal.getInvestigatorInfo().getPi().equals(investigatorRefAndPosition)) {
							existingProposal.getInvestigatorInfo().setPi(investigatorRefAndPosition);
							if (!addedInvestigators.getPi().equals(investigatorRefAndPosition)) {
								addedInvestigators.setPi(investigatorRefAndPosition);
							}
						}
					} else {
				...
				case "1":
					if (!proposalID.equals("0")) {
						...
						if (!existingInvestigators.getCo_pi().contains(investigatorRefAndPosition)) {
		...
		if (proposalInfo != null && proposalInfo.has("OSPSectionInfo")) {
			...
			if (oSPSectionInfo != null && oSPSectionInfo.has("ProgramNo")) {
				if (!proposalID.equals("0")) {
					if (!existingProposal.getoSPSectionInfo().getProgramNo()
						...
						existingProposal.getoSPSectionInfo().setProgramNo(oSPSectionInfo.get("ProgramNo").textValue());
					}
				}
			}
			...
			if (oSPSectionInfo != null && oSPSectionInfo.has("ProgramTitle")) {
				...
				if (oSPSectionInfo != null && oSPSectionInfo.has("PIFringe")) {
					...
					newOSPSectionInfo.setPIFringe(Double.parseDouble(oSPSectionInfo.get("PIFringe").textValue()));
				}
			}
			...
			if (oSPSectionInfo != null && oSPSectionInfo.has("DepartmentId")) {
				if (oSPSectionInfo != null && oSPSectionInfo.has("PIFringe")) {
					...
					newOSPSectionInfo.setDepartmentId(oSPSectionInfo.get("DepartmentId").textValue());
				}
			}
		...
		String projectTitle = currentProposal.getProjectInfo().getProjectTitle();
		if (addedPI.getUserRef().getId() != null) {
			...
			notification.setUsername(addedPI.getUserRef().getUserAccount().getUserName());
		...
		for (InvestigatorRefAndPosition senior : addedInvestigators.getSeniorPersonnel()) {
			notification = new NotificationLog();
			notification.setType("Investigator");
			notification.setAction("Added as Senior Personnel by " + authorUserName + ".");
			notification.setProposalId(currentProposal.getId().toString());
			notification.setProposalTitle(projectTitle);
		...
		if (!deletedPI.getUserProfileId().equals("")) {
			...
			notification.setUsername(deletedPI.getUserRef().getUserAccount().getUserName());
		...
		for (InvestigatorRefAndPosition senior : deletedInvestigators.getSeniorPersonnel()) {
			notification = new NotificationLog();
			notification.setType("Investigator");
			notification.setAction("Deleted as Senior Personnel by " + authorUserName + ".");
			notification.setProposalId(currentProposal.getId().toString());
			notification.setProposalTitle(projectTitle);
			notification.setUserProfileId(senior.getUserProfileId());
			notification.setUsername(senior.getUserRef().getUserAccount().getUserName());
		...
		if (addedSignatures.size() != 0) {
			for (SignatureInfo signatureInfo : addedSignatures) {
				...
				notification.setAction("Signed by " + signFullName + " as " + position + ".");
				...
				notification.setProposalId(currentProposal.getId().toString());
				notification.setProposalTitle(projectTitle);
				notification.setUserProfileId(currentProposal.getInvestigatorInfo().getPi().getUserProfileId());
				notification.setUsername(
						currentProposal.getInvestigatorInfo().getPi().getUserRef().getUserAccount().getUserName());
				notification.setCollege(currentProposal.getInvestigatorInfo().getPi().getCollege());
				notification.setDepartment(currentProposal.getInvestigatorInfo().getPi().getDepartment());
				notification.setPositionType(currentProposal.getInvestigatorInfo().getPi().getPositionType());
				notification.setPositionTitle(currentProposal.getInvestigatorInfo().getPi().getPositionTitle());
				notificationDAO.save(notification);

				for (InvestigatorRefAndPosition copi : currentProposal.getInvestigatorInfo().getCo_pi()) {
					notification = new NotificationLog();
					notification.setType("Signature");
					notification.setAction("Signed by " + signFullName + " as " + position + ".");
					notification.setProposalId(currentProposal.getId().toString());
				...
				for (InvestigatorRefAndPosition senior : currentProposal.getInvestigatorInfo().getSeniorPersonnel()) {
					notification = new NotificationLog();
					notification.setType("Signature");
					notification.setAction("Signed by " + signFullName + " as " + position + ".");
					notification.setProposalId(currentProposal.getId().toString());
			...
			if (policyInfo != null && policyInfo.isArray()) {
				...
				if (root != null && root.has("proposalId")) {
					...
					if (!proposalId.equals("")) {
						ObjectId id = new ObjectId(proposalId);
						Proposal proposal = proposalDAO.findProposalByProposalID(id);
						resourceMap.put("status", proposal.getProposalStatus().get(0).toString());
						attrMap.put("Resource", resourceMap);
					}
				}
				...
				if (decision.equals("Permit")) {
					return Response.status(200).type(MediaType.TEXT_HTML).entity(true).build();
				} else {
					return Response.status(403).type(MediaType.TEXT_HTML).entity("Your permission is: " + decision)
							.build();
				}
			} else {
				return Response.status(403).type(MediaType.TEXT_HTML).entity("No User Permission Attributes are send!")
						.build();
			}
		} else {
			return Response.status(403).type(MediaType.TEXT_HTML).entity("No User Permission Attributes are send!")
