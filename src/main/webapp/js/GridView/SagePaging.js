var sagePaging = {
	...
	NavPaging : function(pageNumber, pageToShow) {
		...
		if (pageNumber > 1) {
			...
			prev = '<a href="#" alt="' + page
					+ '" class="cssClassPagePrevNxt">&lsaquo;</a>';
		...
		for (var i = startPage; i <= endPage; i++) {
			if (pageNumber == i) {
				this.nav += '<span class="cssClassPaginationActive">' + i
						+ '</span>';
			} else {
		...
		if (pageNumber < parseInt(this.maxPage)) {
			page = parseInt(pageNumber) + 1;
			next = '<a href="#" alt="' + page
					+ '" class="cssClassPagePrevNxt">&rsaquo;</a>';
			last = '<a href="#" alt="' + this.maxPage
					+ '" class="cssClassPagePrevNxt">&raquo;</a>';
		} else {
			next = '';
			last = '';
		}
