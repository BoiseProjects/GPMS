	...
	$.createGrid = function(t, p) {
		...
		var g = {
			wcf : function() {
												...
												.ajax({
													...
													complete : function() {
														...
														if (p.runAfterCaseValue != "") {
															g
																	.runAfterCase(p.runAfterCaseValue);

															window[p.runAfterCaseValue]
																	(p.runAfterCaseArguments);
														}
						...
						for (j = 0; j < p.colModel.length; j++) {
							...
							if (cm.coltype == 'checkbox') {
								...
								$('.' + elementclass).bind(
										'click',
										function() {
											g.MakeHeaderCheck(elementclass,
													controlClass);
										});
							}
						}
				...
				// if (target.attr("class") !== 'Sageshowhide
				// cssClassActionImg')
			...
			newPagination : function() {
				...
				var txt = p.pageOf.replace(/{box}/,
						'<input type="text" value="' + p.current
								+ '" readonly="readonly" />');
						...
						.keypress(
								function(e) {
									...
									if (code == 13) { // Enter keycode
										if (txtval != ''
												&& (parseInt(txtval) <= parseInt(sagePaging.maxPage)))
			...
			addActions : function() {
				if (p.buttons) {
					$('tbody tr', t)
							.each(
									function(k) {
										var actionwrapper = document
												.createElement('div');
										var actiontoolswrapper = document
												.createElement('div');
										var actiontools = document
												.createElement('div');
										var actionsP = document
												.createElement('p');
										var showhide = document
												.createElement('p');
										var currentIndex = $(this)
												.closest("tr")[0].rowIndex - 1;
										$(actionwrapper).addClass(
												'cssClassActionOnClick');
										$(actiontoolswrapper).addClass(
												'cssClassActionOnClickShow');
										$(actiontools)
												.addClass(
										...
										$(showhide)
												.append(
														'<a href="javascript:;" class="icon-settings">&nbsp;</a>');
										// $(showhide).append('<a href=#edit' +
										// currentIndex + '>&nbsp;</a>');

										// $(actiontoolswrapper).css({ width:
										// '200px', height: '200px', border:
										// '1px solid red' })
														...
														function(rcount) {
															...
															case "checkbox":
																...
																if (tdcheckedItems[rcount] != undefined) {
																	...
																	var checkID = $(
																			...
																			.find(
																					'TD:eq("'
																							+ tdcheckedItems[rcount]
																							+ '")')
																			.text();
																	// alert(checkID);
																	enableCheckStatus = (checkID
																			.toLowerCase() == 'yes') ? 'checked'
																			: '';
															...
															case "linklabel":
																...
																$(lblVal)
																		.click(
																				function() {
																					if (DownloadMethod[rcount] != undefined
																							&& DownloadArguments[rcount] != undefined) {
																						g
																								.AddLabelLinkEvent(
																										trElement,
																										$(this),
																...
																if (ControlClass[rcount] != undefined) {
																	if (tdRandomValue[rcount] != undefined) {
																		...
																		var randomValue = $(
																				...
																				.find(
																						'TD:eq("'
																								+ tdRandomValue[rcount]
																								+ '")')
																				.text();
																		lblVal
																				.addClass(ControlClass[rcount]
																						+ '_'
																						+ randomValue);
																	}
															...
															case "image":
																...
																if (p.imageOfType != '') {
																	...
																	case 'item': {
																		previousvalue = "Modules/AspxCommerce/AspxItemsManagement/uploads/Small/"
																				+ previousvalue;
																	}
																		break;
																	case 'category': {
																		previousvalue = "Modules/AspxCommerce/AspxCategoryManagement/uploads/Small/"
																				+ previousvalue;
																	}
																		break;
																	case 'payment': {
																		previousvalue = "Modules/AspxCommerce/AspxPaymentGateWayManagement/uploads/Small/"
																				+ previousvalue;
																	}
																		break;
																	case 'currency': {
																		previousvalue = "images/flags/"
																				+ previousvalue;
																	}
																		break;
																	default:
			...
			getCellValue : function(ele, colIndex) {
				...
				var indexValue = $(parentEls).find('TD:eq("' + colIndex + '")')
						.text();
				if (!indexValue)
					indexValue = '';
				return indexValue;
			...
			AddLabelLinkEvent : function(trElement, lblVal, index) {
				...
				if (arguments != '') {
					...
					for (loop = 0; loop < argus.length; loop++) {
						arg[arg.length] = g
								.getCellValue($(lblVal), argus[loop]);
					}
				}
				...
				if (tdURL[index] != 'item' && tdURL[index] != 'category') {
					if (ShowPopUp[index]) {
						...
						if (arguments != '') {
							var argus = arguments.split(',');
							for (loop = 0; loop < argus.length; loop++) {
								arg[arg.length] = g.getCellValue($(href),
										argus[loop]);
							}
						}
						getPopUpFunction(PopUpMethod[index], arg,
								PopUpID[index]);
					} else {
						...
						if (queryPairs != '') {
							...
							$.each(opt, function(i, item) {
								...
								var queryValue = g.getCellValue(trElement,
										optValue[1]);
								queryString += querySymbol + optValue[0] + '='
										+ queryValue;
							});
					...
					var queryPairs = QueryPairs[index];
					var queryString = '';
					// Updated on August-2-2012 to implement costVariant
					// redirection to item detail page.
			...
			AddButtonEvent : function(trElement, btn, index) {
				if (ShowPopUp[index]) {
					...
					if (arguments != '') {
						var argus = arguments.split(',');
						for (loop = 0; loop < argus.length; loop++) {
							arg[arg.length] = g.getCellValue($(btn),
									argus[loop]);
						}
					}
