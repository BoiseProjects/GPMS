	...
	$.format = (function() {
		...
		function parseTime(value) {
			...
			if (timeArray.length === 3) {
				...
				// => 20
				second = timeArray[2].replace(/\s.+/, '')
						.replace(/[a-z]/gi, '');
				// '01:10:20 GMT-0200 (BRST)'.replace(/\s.+/,
				// '').replace(/[a-z]/gi, '');
				// => 01:10:20
		...
		return {
			...
			parseDate : function(value) {
				...
				if (typeof value == 'number') {
					...
					if (values.length === 6 && isNaN(values[5])) {
						// values[5] == year
						/*
						 * This change is necessary to make `Mon Apr 28 2014
						 * 05:30:00 GMT-0300` work like `case 7` otherwise it
						 * will be considered like `Wed Jan 13 10:43:41 CET 2010
						 * Fixes:
					...
					case 9:
						/*
						 * added by Larry, for Fri Apr 08 2011 00:00:00 GMT+0800
						 * (China Standard Time)
						 */
					case 10:
						/*
						 * added by Larry, for Fri Apr 08 2011 00:00:00 GMT+0200
					...
					case 1:
						/*
						 * added by Jonny, for 2012-02-07CET00:00:00 (Doctrine
						 * Entity -> Json Serializer)
						 */
						subValues = values[0].split('');
						parsedDate.year = subValues[0] + subValues[1]
								+ subValues[2] + subValues[3];
						parsedDate.month = subValues[5] + subValues[6];
						parsedDate.dayOfMonth = subValues[8] + subValues[9];
						parsedDate.time = parseTime(subValues[13]
								+ subValues[14] + subValues[15] + subValues[16]
								+ subValues[17] + subValues[18] + subValues[19]
								+ subValues[20]);
						break;
					default:
						return null;
				...
				if (parsedDate.time) {
					parsedDate.date = new Date(parsedDate.year,
							parsedDate.month - 1, parsedDate.dayOfMonth,
							parsedDate.time.hour, parsedDate.time.minute,
							parsedDate.time.second, parsedDate.time.millis);
				} else {
					parsedDate.date = new Date(parsedDate.year,
							parsedDate.month - 1, parsedDate.dayOfMonth);
				}
				...
				try {
					...
					for (var i = 0; i < format.length; i++) {
						...
						case 'hh':
							 ...
							 * ===
							 */
							hour = (parseInt(time.hour, 10) === 0 ? 12
			 ...
			 * 
			 * Takes an ISO time and returns a string representing how long ago
			 * the date represents
			 * 
