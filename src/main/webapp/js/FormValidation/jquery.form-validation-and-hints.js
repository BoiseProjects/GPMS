...
function isTypeValidExt(classprefix, type, value) {
	/*
	 * RULE EXAMPLE (Accept only integer values) if( type == classprefix +
	 * 'Integer' ) { return ( ( value.match(/^[\d|,|\.|\s]*$/) ) && ( value != '' ) ); }
	 */
	return true;
}
				...
				function() {
					...
					for (var i = 0; i < document.forms.length; i++) {
						...
						for (var j = 0; j < fe.length; j++) {
							...
							} else if (((fe[j]).type == "text"
									|| (fe[j]).type == "password" || (fe[j]).tagName == 'TEXTAREA')
									&& (fe[j]).title.indexOf("*") == 0) {
								addHint((fe[j]));
...
function addHint(field) {
	...
	if (field.value == "" || field.value == titleHint) {
		// in "password" inputs, set to "text" to show hint, preserve type in
		// class attribute
		if (field.type == "password") {
			$(field).addClass("password");
			var newObject = changeInputType(field, "text");// returns false for
															// non-ie
...
function removeHint(field) { // only on INPUT.text items
	...
	if (field.value == "" || field.value == titleHint) {
		...
		if ($(field).hasClass("password")) {
			var newObject = changeInputType(field, "password"); // returns false
																// for non-ie
...
function isValidDate(date) {
	...
	// (month == 11)) && (day > 30)) valid = false;
	// else if ((month == 1) && (((year % 400) == 0) || ((year % 4) == 0)) &&
	// ((year % 100) != 0) && (day >= 29)) valid = false;
	// else if ((month == 1) && ((year % 100) == 0) || (day > 28)) valid =
	// false;
	// return valid;
}
...
function checkForm(form) {
	...
	$('.required').find('.cke_skin_v2 iframe').each(
			function() {
				if ($(this).contents().find("body").text() != ''
						&& $(this).contents().find("body").text() != null) {
					$(this).parents('.required').find('.iferror').html('');
					$(this).parents().removeClass("diverror");
				} else {

					$(this).parents('.required').find('.iferror').html(
							getLocale(CoreJsLanguage, "Mandatory Fields"));
					$(this).parents('table').parent('span').parent('span')
							.parent('span').parent('div').addClass("diverror");
	...
	if ($('.activeto').length != 0) {
		if (isValidDate($('.activefrom').val())) {
			if (isValidDate($('.activeto').val())) {
				if (Date.parse($('.activefrom').val()) > Date.parse($(
						'.activeto').val())) {
					if (send) {
						moveTo($('.activeto'));
					}
					...
					$('.activeto')
							...
							.html(
									getLocale(CoreJsLanguage,
											'Active To date must be higher date than Active From date!'));
					send = false;
					// $(To[vt]).css("background-color","#FFCA85");
				}
			} else {
				showErrorOn($('.activeto'));
				$('.activeto').parents('.required').find('.iferror').html(
						getLocale(CoreJsLanguage,
								'You have entered invalid Active To date!'));
				send = false;
			}
		} else {
			showErrorOn($('.activefrom'));
			$('.activefrom').parents('.required').find('.iferror').html(
					getLocale(CoreJsLanguage,
							'You have entered invalid Active From date!'));
			send = false;
		}
	}
		...
		if ($('.SpecialDropDown').val() == 5) {
			if (isValidDate($('.classSpecialFrom').val())) {
				if (isValidDate($('.classSpecialTo').val())) {
					if (Date.parse($('.classSpecialFrom').val()) > Date
							.parse($('.classSpecialTo').val())) {
						if (send) {
							moveTo($('.classSpecialTo'));
						}
						showErrorOn($('.classSpecialTo'));
						showErrorOn($('.classSpecialFrom'));
						// $('.classSpecialTo').parents('.required').find('.iferror').html('Special
						// To date must be higher date than Special From
						// date!');
						$('.classSpecialTo')
								.next('span')
								.html(
										'Special To date must be higher date than Special From date!');
						send = false;
						// $(To[vt]).css("background-color","#FFCA85");
					}
				} else {
					showErrorOn($('.classSpecialTo'));
					// $('.classSpecialTo').parents('.required').find('.iferror').html('You
					// have entered invalid Active To date!');
					$('.classSpecialTo').next('span').html(
							'You have entered invalid Special To date!');
					send = false;
				}
			} else {
				showErrorOn($('.classSpecialFrom'));
				// $('.classSpecialFrom').parents('.required').find('.iferror').html('You
				// have entered invalid Active From date!');
				$('.classSpecialFrom').next('span').html(
						'You have entered invalid Special From date!');
				send = false;
			}
		} else {
			send = true;
		...
		if ($('.classSpecialPriceTo').length != 0) {
			if (isValidDate($('.classSpecialPriceFrom').val())) {
				if (isValidDate($('.classSpecialPriceTo').val())) {
					if (Date.parse($('.classSpecialPriceFrom').val()) > Date
							.parse($('.classSpecialPriceTo').val())) {
						if (send) {
							moveTo($('.classSpecialPriceTo'));
						}
						...
						$('.classSpecialPriceTo')
								.next('span')
								.html(
										'Special Price To date must be higher date than Special Price From date!');
						send = false;
						return false;
					}
				} else {
					showErrorOn($('.classSpecialPriceTo'));
					$('.classSpecialPriceTo').next('span').html(
							'You have entered invalid Special To date!');
					send = false;
					return false;
				}
				...
				showErrorOn($('.classSpecialPriceFrom'));
				$('.classSpecialPriceFrom').next('span').html(
						'You have entered invalid Special From date!');
				send = false;
				return false;
			}
		...
		if ($('.FeaturedDropDown').val() == 3) {
			if (isValidDate($('.classFeaturedFrom').val())) {
				if (isValidDate($('.classFeaturedTo').val())) {
					if (Date.parse($('.classFeaturedFrom').val()) > Date
							.parse($('.classFeaturedTo').val())) {
						if (send) {
							moveTo($('.classFeaturedTo'));
						}
						...
						$('.classFeaturedTo')
								.next('span')
								.html(
										'Featured To date must be higher date than Featured From date!');
						send = false;
						// $(To[vt]).css("background-color","#FFCA85");
					}
				} else {
					...
					$('.classFeaturedTo').next('span').html(
							'You have entered invalid Featured To date!');
					send = false;
				}
			} else {
				showErrorOn($('.classFeaturedFrom'));
				$('.classFeaturedFrom').next('span').html(
						'You have entered invalid Featured From date!');
				send = false;
			}
		} else {
			send = true;
	...
	if ($('.classActiveTo').length != 0) {
		if (isValidDate($('.classActiveFrom').val())) {
			if (isValidDate($('.classActiveTo').val())) {
				if (Date.parse($('.classActiveFrom').val()) > Date.parse($(
						'.classActiveTo').val())) {
					if (send) {
						moveTo($('.classActiveTo'));
					}
			...
			showErrorOn($('.classActiveFrom  '));
			$('.classActiveFrom ').next('span').html(
					getLocale(CoreJsLanguage,
							"You have entered invalid Active From date!"));
			send = false;
		}
		...
		if ($.trim($('.classNewFrom').val()) != '') {
			if (isValidDate($('.classNewFrom').val())) {
				if (isValidDate($('.classNewTo').val())) {
					if (Date.parse($('.classNewFrom').val()) > Date.parse($(
							'.classNewTo').val())) {
						if (send) {
							moveTo($('.classNewTo'));
						}
						...
						$('.classNewTo')
								.next('span')
								.html(
										'New To date must be higher date than From date!');
						send = false;
						// $(To[vt]).css("background-color","#FFCA85");
					}
				} else {
					showErrorOn($('.classNewTo'));
					$('.classNewTo').next('span').html(
							'You have entered invalid New To date!');
					send = false;
				}
			} else {
				showErrorOn($('.classNewFrom'));
				$('.classNewFrom').next('span').html(
						'You have entered invalid New From date!');
				send = false;
			}
		}
	}
	...
	inputs = $(form).find(
			'INPUT[class*="' + classprefix
					+ '"], INPUT:not(:file), TEXTAREA, .required SELECT')
			.filter(':not(:disabled)').not('.class-text');
					...
					function(i, val) {
						...
						case 'radio':
							if (radioGroups != undefined) {
								if (window.radioGroups[input.prop('name')] === undefined) {
									radioGroups[input.prop('name')] = new Array();
								}
								radioGroups[input.prop('name')][radioGroups[input
										.prop('name')].length] = input;
							}
							break;
						...
						case 'password':
							...
							break;
						case 'textarea':
							if ((isFilled(input) || isRequired(input))
									&& (!isValid(input))) {
								if (send) {
									moveTo(input);
								}
...
function isValid(input) {
	...
	while (result) {
		if (start == -1
				...
				|| string.charAt((start + classprefix.length)) != string
						.charAt((start + classprefix.length)).toUpperCase()) {
			break;
		} else {
...
function isTypeValid(input, type, value) {
	...
	if (type == classprefix + 'Url') {
		return (value
				.match(/^(https?:\/\/)?(([0-9a-z_!~*'().&=+$%-]+: )?[0-9a-z_!~*'().&=+$%-]+@)?(([0-9]{1,3}\.){3}[0-9]{1,3}|([0-9a-z_!~*'()-]+\.)*([0-9a-z][0-9a-z-]{0,61})?[0-9a-z]\.[a-z]{2,6})(:[0-9]{1,4})?((\/?)|(\/[0-9a-z_!~*'().;?:@&=+$,%#-]+)+\/?)$/));
	}
...
function showErrorOn(input) {
	/*
	 * BUG FIXED FOR IE: when submited it auto focuses to the first required
	 * field, so the hint and red box aren't there. Might be confusing to a
	 * user!
	 * 
	 * input.bind('focus.rmErrorClass', function(){ rmErrorClass( this ); });
	 */
...
function rmErrorClass(elm) {
	...
	$(elm).unbind('.rmErrorClass'); // no further clicks will trigger
									// rmErrorClass();
	if (etag) {
