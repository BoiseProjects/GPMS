...
(function() {
	...
	if (m < 5.5) {
		...
		J += "address,blockquote,body,dd,div,dt,fieldset,form,"
	...
	IE7._f = function(a, b) {
		if (b == "src" && a.pngSrc)
			return a.pngSrc;
		var c = bi ? (a.attributes[b] || a.attributes[bG[b.toLowerCase()]]) : a
				.getAttributeNode(b);
	...
	var ce = H
			.extend({
				...
				format : function(a) {
					return a.replace(cY, "$1").replace(cX, "$1 $2").replace(cW,
							"$1*$2")
				},
				optimise : function(a) {
	...
	IE7.CSS = new (bZ.extend({
		parser : new bU,
		screen : "",
		print : "",
		styles : [],
		rules : [],
		pseudoClasses : m < 7 ? "first\\-child" : "",
	...
	var bJ = B.extend({
		...
		getText : function() {
			...
			function cy(a, b) {
				...
				if (bw[c])
					return "";
				bw[c] = (a.disabled) ? "" : cA(IE7.CSS.getText(a, b), by(
						a.href, b));
				return bw[c]
			}
			;
			var cz = /(url\s*\(\s*['"]?)([\w\.]+[^:\)]*['"]?\))/gi;
			function cA(a, b) {
				return a.replace(cz, "$1" + b.slice(0, b.lastIndexOf("/") + 1)
						+ "$2")
			}
			;
	...
	if (m < 7) {
		var Z = new A("hover", function(a) {
			...
			IE7.CSS.addEventHandler(a,
					m < 5.5 ? "onmouseover" : "onmouseenter", function() {
						Z.register(b)
					});
			IE7.CSS.addEventHandler(a, m < 5.5 ? "onmouseout" : "onmouseleave",
					function() {
						Z.unregister(b)
					})
		});
		...
		function collapseMargin(a, b) {
			if (!a.runtimeStyle[b]) {
				...
				if (c && IE7.hasLayout(c)
						&& !IE7[b == "marginTop" ? "_4" : "_3"](a))
					return;
				var d = x(">*:" + (b == "marginTop" ? "first" : "last")
						+ "-child", a, true);
		...
		var fixWidth = function(g) {
			...
			function h(a, b) {
				if (!a.runtimeStyle.fixedWidth) {
					...
					a.runtimeStyle.fixedWidth = (UNIT.test(b)) ? Math.max(0, r(
							a, b)) : b;
			...
			;
			function u(a, b) {
				return q(a, a.currentStyle[b + "Left"])
						+ q(a, a.currentStyle[b + "Right"])
			}
			;
			...
			;
			function s(a) {
				if (X(s, a, /^(fixed|absolute)$/.test(a.currentStyle.position)
						&& bF(a, "left") != "auto" && bF(a, "right") != "auto"
						&& AUTO.test(bF(a, "width")))) {
					R(a);
			...
			bC(function() {
				...
				for (a in c) {
					var d = c[a];
					var f = (parseInt(d.runtimeStyle.width) == r(d,
	...
	var bk = bc("/SageFrameTemplate/blank.gif", cC);
	...
	if (m >= 5.5 && m < 7) {
		IE7.CSS.addFix(
				...
				function(a, b, c, d, f) {
					d = bB(d);
					return bm.test(d) ? "filter:" + i(bK, d, "crop")
							+ ";zoom:1;background" + (b || "") + ":"
							+ (c || "") + "none" + (f || "") : a
				});
	...
	new function(_) {
		...
		;
		function _5(a) {
			if (X(_5, a, a.currentStyle.backgroundAttachment == "fixed"
					&& !a.contains(w))) {
				_6();
				bgLeft(a);
				bgTop(a);
		...
		function getOffsetLeft(a, b) {
			...
			default:
				if (M.test(b)) {
					return parseInt((t.clientWidth - _0.offsetWidth)
							* parseFloat(b) / 100)
				}
				_0.style.left = b;
		...
		function _7(a, b) {
			...
			if (!a.runtimeStyle.autoLeft && a.currentStyle.marginLeft == "auto"
				...
				var c = t.clientWidth - getPixelWidth(a, a.currentStyle.right)
		...
		function getPixelWidth(a, b) {
			return M.test(b) ? parseInt(parseFloat(b) / 100 * t.clientWidth)
					: D(a, b)
		}
		;
	...
	function cq() {
		...
		;
		function s(a) {
			return D(a, a.currentStyle.paddingLeft)
					+ D(a, a.currentStyle.paddingRight)
		}
		;
		IE7.CSS.addRecalc("margin(-left|-right)?", "[^};]*auto", function(a) {
			if (X(n, a, a.parentElement && a.currentStyle.display == "block"
	...
	function cr(a, b, c, d) {
		...
		var j = i(f == 0 ? "%3%7" + (d + g) : "(%4%3-%2)%6%1%70%5%4%3>=%2", f,
				g, c, d, "&&", "%", "==");
		if (h)
			j = "!(" + j + ")";
		return j
	...
	bH = {
		...
		"first-child" : "!IE7._4(e%1)",
		"lang" : "IE7._h(e%1,'%2')",
		"last-child" : "!IE7._3(e%1)",
		"only-child" : "!IE7._4(e%1)&&!IE7._3(e%1)",
	...
	E
			.merge({
				":not\\((\\*|[\\w-]+)?([^)]*)\\)" : function(a, b, c) {
					var d = (b && b != "*") ? i("if(e%1.nodeName=='%2'){", l, b
							.toUpperCase()) : "";
					d += E.exec(c);
	...
	IE7.CSS.pseudoClasses += "before|after|last\\-child|only\\-child|empty|root|"
			+ "not|nth\\-child|nth\\-last\\-child|contains|lang".split("|")
					.join(bM + "|") + bM;
	bV.add(/::/, ":");
	...
	var F = IE7.PseudoElement = o.extend({
		...
		create : function(a) {
			...
			if (b) {
				...
				a.insertAdjacentHTML(h, i(F.ANON, this.className, j, g, f ? ""
						: c));
				if (f) {
	...
	IE7.HTML.addRecalc("form",
			function(c) {
				v(c, "onsubmit",
						function() {
							for (var a, b = 0; a = c[b]; b++) {
								if (cw.test(a.type) && !a.disabled
										&& !a.runtimeStyle.clicked) {
									a.disabled = true;
									setTimeout("document.all." + a.uniqueID
											+ ".disabled=false", 1)
	...
	(function() {
		...
		bm = new RegExp(W(typeof IE7_PNG_SUFFIX == "string" ? IE7_PNG_SUFFIX
				: ".png")
				+ "$", "i");
		w = document.body;
		t = G ? w : bx;
