...
$(function() {
	DashBoard = {
		config : {
			...
			url : "",
			ajaxCallMode : 0,
			baseURL : gpmsAppPath + "/Services/SageFrameGlobalWebService.asmx/"
		},
		vars : {
			numberRegex : /^[+-]?\d+(\.\d+)?([eE][+-]?\d+)?$/,
			adminimagepath : gpmsAppPath
					+ "/Administrator/Templates/Default/images/"
		},
		init : function() {
		...
		utils : {
			...
			GetImagePath : function(imagepath) {
				var fullpath = DashBoard.utils.getapplicationname() + imagepath;
				return fullpath;
			},
			...
			IsNumber : function(str) {
				var isnum = false;
				if (DashBoard.vars.numberRegex.test(str)) {
					isnum = true;
				}
				return isnum;
			...
			GetAdminImage : function(imagename) {
				return (DashBoard.vars.adminimagepath + imagename);
			},
		...
		messaging : {
			show : function(message, messagetype) {
				if (MsgTemplate == "default") {
					...
					messagewrapper += "<span  id='spnMessage' runat='server'>"
							+ message + "</span>";
					$('div.sfMessagewrapper:first').html(messagewrapper)
							.animate(0, function() {
			...
			GetLocalizedMessage : function(culturecode, modulename, messagetype) {
				...
				$.ajax({
					...
					contentType : DashBoard.config.contentType,
					cache : DashBoard.config.cache,
					url : DashBoard.config.baseURL + "GetLocalizedMessage",
					data : param,
					dataType : DashBoard.config.dataType,
					async : false,
		...
		tooltip : {
			consts : {
				helpImage : (gpmsAppPath == "/" ? "" : gpmsAppPath)
						+ "/Administrator/Templates/Default/images/help.png"
			},
			InitializeToolTips : function() {
				this.GetHelpToolTip("imgToolTip", "#testTooltip",
						"This is my tooltip");
			},
			GetHelpToolTip : function(id, container, message) {
				$(container).html(DashBoard.tooltip.BuildTooltipImage(id));
				$.toolTipRequest(id, message, "dark");

			},
			GetToolTip : function(id, container, message) {
				$(container).append(DashBoard.tooltip.BuildTooltipImage(id));
				$.toolTipRequest(id, message, "dark");
			},
			...
			GetTextBoxToolTipImage : function(id, message) {
				$('#' + id).parent().append(
						DashBoard.tooltip.BuildTooltipImage('img_' + id));
...
$(function() {
	...
	$('div.sfModuleinfo div.sfLinks a.sfNclose')
			.bind(
					"click",
					function() {
						var _ModuleID = $(this).prop("id")
								.replace("close_", "");
						var self = $(this);
						$
								.ajax({
									...
									cache : false,
									url : gpmsAppPath
											+ "/Modules/Admin/ModuleMessage/services/ModuleMessageWebService.asmx/UpdateMessage",
