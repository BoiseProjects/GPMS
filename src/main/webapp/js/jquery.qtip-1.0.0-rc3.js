...
(function($) {
	...
	$.fn.qtip = function(options, blanket) {
		...
		// Validate provided options
			...
			if (typeof options.content !== 'object'
					|| (options.content.jquery && options.content.length > 0))
				options.content = {
			...
			// Inherit all style properties into one syle object and include
			// original options
		...
		return $(this)
				.each(
						function() // Return original elements as per jQuery
									// guidelines
						{
							...
							if (typeof options == 'string') {
								...
								// Make sure API data
								// exists$('.qtip').qtip('destroy')
								if (typeof interfaces == 'object') {
									// Check if API call is a BLANKET DESTROY
									// command
									if (blanket === true
											&& command == 'destroy')
									...
									// API call is not a BLANKET DESTROY command
										...
										// Check if supplied command effects
										// this tooltip only (NOT BLANKET)
										if (blanket !== true)
											interfaces = [ $(this).qtip('api') ];

										// Execute command on chosen qTips
										for (i = 0; i < interfaces.length; i++) {
											// Destroy command doesn't require
											// tooltip to be rendered
											if (command == 'destroy')
												interfaces[i].destroy();

											// Only call API if tooltip is
											// rendered and it wasn't a destroy
											// call
							...
							// No API commands, continue with qTip creation
								...
								// Determine tooltip ID (Reuse array slots if
								// possible)
								...
								// Check if element already has qTip data
								// assigned
								if (typeof $(this).data('qtip') == 'object') {
									// Set new current interface id
									if (typeof $(this).attr('qtip') === 'undefined')
										$(this).data('qtip').current = $(this)
												.data('qtip').interfaces.length;

									// Push new API interface onto interfaces
									// array
								...
								// If prerendering is disabled, create tooltip
								// on showEvent
								if (config.content.prerender === false
										&& config.show.when.event !== false
										&& config.show.ready !== true) {
									config.show.when.target
											.bind(
													config.show.when.event
															+ '.qtip-' + id
															+ '-create',
													{
														qtip : id
													...
													function(event) {
														// Retrieve API
														// interface via passed
														// qTip Id
														api = $.fn.qtip.interfaces[event.data.qtip];

														// Unbind show event and
														// cache mouse coords
														api.options.show.when.target
																.unbind(api.options.show.when.event
																		+ '.qtip-'
																		+ event.data.qtip
																		+ '-create');
														api.cache.mouse = {
														...
														construct.call(api);
														api.options.show.when.target
																.trigger(api.options.show.when.event);
													});
								}

								// Prerendering is enabled, create tooltip now
								else {
									// Set mouse position cache to top left of
									// the element
									obj.cache.mouse = {
				...
				.extend(
						...
						self.options.api,
						{
							show : function(event) {
								...
								// Make sure tooltip is rendered and if not,
								// return
								if (!self.status.rendered)
									return $.fn.qtip.log.error
											.call(
													self,
													2,
								...
								// Call API method and if return value is false,
								// halt
								...
								function afterShow() {
									// Call API method and focus if it isn't
									// static
									if (self.options.position.type !== 'static')
										self.focus();
									self.onShow.call(self, event);

									// Prevent antialias from disappearing in
									// IE7 by removing filter attribute
									if ($.browser.msie)
										self.elements.tooltip.get(0).style
								...
								if (typeof self.options.show.effect.type == 'function') {
									...
									case 'grow':
										...
										break;
									default:
										self.elements.tooltip.show(null,
												afterShow);
										break;
									}
									;
									...
									self.elements.tooltip
											.addClass(self.options.style.classes.active);
								}
								;
							...
							hide : function(event) {
								...
								if (typeof self.options.hide.effect.type == 'function') {
									...
									case 'grow':
										...
										break;
									default:
										self.elements.tooltip.hide(null,
												afterHide);
										break;
									}
									;
									...
									self.elements.tooltip
											.removeClass(self.options.style.classes.active);
								}
								;
							...
							updatePosition : function(event, animate) {
								...
								if (self.options.position.target !== 'mouse') {
									// If the HTML element is AREA, calculate
									// position manually
									if (self.options.position.target.get(0).nodeName
											.toLowerCase() == 'area') {
										// Retrieve coordinates from coords
										// attribute and parse into integers
										coords = self.options.position.target
												.attr('coords').split(',');
										for (i = 0; i < coords.length; i++)
											coords[i] = parseInt(coords[i]);

										// Setup target position object
										mapName = self.options.position.target
										...
										// area
										switch (self.options.position.target
												.attr('shape').toLowerCase()) {
										case 'rect':
											target.dimensions = {
												width : Math.ceil(Math
														.abs(coords[2]
																- coords[0])),
										...
										case 'poly':
											...
											target.dimensions.width = target.dimensions.width
													- (target.position.left - imagePos.left);
									...
									else if (self.options.position.target
									...
									// Target is a regular HTML element, find
									// position normally
									else {
										// Check if the target is another
										// tooltip. If its animated, retrieve
										// position from newPosition data
										if (typeof self.options.position.target
												.attr('qtip') !== 'undefined')
											target.position = self.options.position.target
										...
										target.dimensions = {
											height : self.options.position.target
													.outerHeight(),
											width : self.options.position.target
													.outerWidth()
										};
									}
									;
								...
								// Mouse is the target, set position to current
								// mouse coordinates
								else {
									// Setup target position and dimensions
									// objects
									target.position = newPosition = {
										left : self.cache.mouse.x,
										top : self.cache.mouse.y
									};
									target.dimensions = {
										height : 1,
										width : 1
								...
								// Setup IE adjustment variables (Pixel gap
								// bugs)
								ieAdjust = ($.browser.msie) ? 1 : 0; // And
																		// this
																		// is
																		// why I
																		// hate
																		// IE...
								ie6Adjust = ($.browser.msie && parseInt($.browser.version
										.charAt(0)) === 6) ? 1 : 0; // ...and
																	// even more
																	// so IE6!
								...
								// If screen adjustment is enabled, apply
								// adjustments
								if (self.options.position.adjust.screen === true)
									newPosition = screenAdjust.call(self,
											newPosition, target, tooltip);

								// If mouse is the target, prevent tooltip
								// appearing directly under the mouse
								if (self.options.position.target === 'mouse'
										&& self.options.position.adjust.mouse === true) {
									if (self.options.position.adjust.screen === true
											&& self.elements.tip)
										mouseAdjust = self.elements.tip
												.attr('rel');
									else
										mouseAdjust = self.options.position.corner.tooltip;

									newPosition.left += (mouseAdjust
											.search(/right/i) !== -1) ? -6 : 6;
									newPosition.top += (mouseAdjust
											.search(/bottom/i) !== -1) ? -6 : 6;
								}

								// Initiate bgiframe plugin in IE6 if tooltip
								// overlaps a select box or object element
									...
									$('select, object')
											.each(
													function() {
														...
														if (newPosition.top
																+ tooltip.dimensions.height >= offset.top
																&& newPosition.left
																		+ tooltip.dimensions.width >= offset.left)
															bgiframe.call(self);
													});
								}
								;
								...
								// Set new tooltip position if its moved,
								// animate if enabled
								curPosition = self.getPosition();
								if (newPosition.left != curPosition.left
										|| newPosition.top != curPosition.top) {
									// Call API method and if return value is
									// false, halt
									returned = self.beforePositionUpdate.call(
											self, event);
									if (returned === false)
										return self;
									...
									if (animate === true) {
										...
										// Animate and reset animated status on
										// animation end
										self.elements.tooltip
												.animate(
														newPosition,
									...
									// Set new position via CSS
										...
										self.elements.tooltip.css(newPosition);

									// Call API method and log event if its not
									// a mouse move
							...
							updateWidth : function(newWidth) {
								...
								// Make sure supplied width is a number and if
								// not, return
								else if (newWidth
								...
								// Setup elements which must be hidden during
								// width update
								hidden = self.elements.contentWrapper
										.siblings().add(self.elements.tip).add(
												self.elements.button);

								// Calculate the new width if one is not
								// supplied
								if (!newWidth) {
									...
									// No width is set, proceed with auto
									// detection
									else {
										// Set width to auto initally to
										// determine new width and hide other
										// elements
										...
										// Set position and zoom to defaults to
										// prevent IE hasLayout bug
										if ($.browser.msie)
										...
										// Make sure its within the maximum and
										// minimum width boundries
										if (!self.options.style.width.value) {
											if (newWidth > self.options.style.width.max)
												newWidth = self.options.style.width.max
											if (newWidth < self.options.style.width.min)
												newWidth = self.options.style.width.min
										}
										;
									}
									;
								...
								// Adjust newWidth by 1px if width is odd (IE6
								// rounding bug fix)
								if (newWidth % 2 !== 0)
									newWidth -= 1;

								// Set the new calculated width and unhide other
								// elements
								...
								// IE only adjustments
								if ($.browser.msie) {
									// Reset position and zoom to give the
									// wrapper layout (IE hasLayout bug)
									self.elements.wrapper.add(
											self.elements.contentWrapper
													.children()).css({
										zoom : '1'
									});
									...
									// Adjust BGIframe height and width if
									// enabled
									if (self.elements.bgiframe)
										self.elements.bgiframe
												.width(newWidth)
												.height(
														self.getDimensions.height);
								}
								;
							...
							updateStyle : function(name) {
								...
								// Return if style is not defined or name is not
								// a string
								...
								// Update initial styles of content and title
								// elements
								self.elements.content
										.css(jQueryStyle(self.options.style));
								if (self.options.content.title.text !== false)
									self.elements.title.css(jQueryStyle(
											self.options.style.title, true));
								...
								if (self.options.style.tip.corner !== false) {
									if ($('<canvas>').get(0).getContext) {
										...
										tip = self.elements.tooltip
												.find('.qtip-tip canvas:first');
										context = tip.get(0).getContext('2d');
										context.clearRect(0, 0, 300, 300);

										// Draw new tip
										corner = tip.parent('div[rel]:first')
												.attr('rel');
								...
								if (self.options.style.border.radius > 0) {
									...
									if ($('<canvas>').get(0).getContext) {
										...
										// Set new fillcolor attribute on each
										// border corner
										self.elements.tooltip
												.find(
							...
							updateContent : function(content, reposition) {
								...
								// Call API method and set new content if a
								// string is returned
								parsedContent = self.beforeContentUpdate.call(
										self, content);
								...
								// Set position and zoom to defaults to prevent
								// IE hasLayout bug
								if ($.browser.msie)
									self.elements.contentWrapper.children()
											.css({
												zoom : 'normal'
											});

								// Append new content if its a DOM array and
								// show it if hidden
								if (content.jquery && content.length > 0)
									content.clone(true).appendTo(
											self.elements.content).show();

								// Content is a regular string, insert the new
								// content
								else
									self.elements.content.html(content);

								// Check if images need to be loaded before
								// position is updated to prevent
								// mis-positioning
								images = self.elements.content
								...
								function afterLoad() {
									...
									// If repositioning is enabled, update
									// positions
									if (reposition !== false) {
										// Update position if tooltip isn't
										// static
										if (self.options.position.type !== 'static')
											self.updatePosition(
													self.elements.tooltip
							...
							updateTitle : function(content) {
								...
								// Set the new content and reappend the button
								// if enabled
								if (self.elements.button)
									self.elements.button = self.elements.button
											.clone(true);
								self.elements.title.html(content)
								if (self.elements.button)
									self.elements.title
											.prepend(self.elements.button);
							...
							focus : function(event) {
								...
								curIndex = parseInt(self.elements.tooltip
										.css('z-index'));
								newIndex = 6000 + $('div.qtip[qtip]').length - 1;

								// Only update the z-index if it has changed and
								// tooltip is not already focused
								if (!self.status.focused
										&& curIndex !== newIndex) {
									// Call API method and if return value is
									// false, halt
									returned = self.beforeFocus.call(self,
											event);
									if (returned === false)
										return self;
									...
									// Set the new z-index and set focus status
									// to true
									self.elements.tooltip.css({
										zIndex : newIndex
									});
							...
							disable : function(state) {
								...
								if (state) {
									...
									// Tooltip is already disabled, inform user
									// via log
									else
										$.fn.qtip.log.error
												.call(
									...
									if (self.status.disabled) {
										// Reassign events, set disable status
										// and log
									...
									// Tooltip is already enabled, inform the
									// user via log
									else
										$.fn.qtip.log.error
												.call(
							...
							destroy : function() {
								...
								if (self.status.rendered) {
									...
									self.options.show.when.target.unbind(
											'mousemove.qtip',
											self.updatePosition);
									self.options.show.when.target.unbind(
											'mouseout.qtip', self.hide);
									self.options.show.when.target
											.unbind(self.options.show.when.event
													+ '.qtip');
									self.options.hide.when.target
											.unbind(self.options.hide.when.event
													+ '.qtip');
									self.elements.tooltip
											.unbind(self.options.hide.when.event
													+ '.qtip');
									self.elements.tooltip.unbind(
											'mouseover.qtip', self.focus);
									self.elements.tooltip.remove();
								}

								// Tooltip isn't yet rendered, remove render
								// event
								else
									self.options.show.when.target
											.unbind(self.options.show.when.event
								...
								// Check to make sure qTip data is present on
								// target element
								if (typeof self.elements.target.data('qtip') == 'object') {
									// Remove API references from interfaces
									// object
									interfaces = self.elements.target
								...
								// Set qTip current id to previous tooltips API
								// if available
								if (typeof interfaces == 'object'
										&& interfaces.length > 0)
							...
							getPosition : function() {
								...
								show = (self.elements.tooltip.css('display') !== 'none') ? false
										: true;

								// Show and hide tooltip to make sure
							...
							getDimensions : function() {
								...
								// Show and hide tooltip to make sure dimensions
								// are returned
								if (show)
									self.elements.tooltip.css({
										visiblity : 'hidden'
	...
	function construct() {
		...
		self.elements.contentWrapper = self.elements.wrapper.children(
				'div:first').css({
			background : self.options.style.background
		});
		self.elements.content = self.elements.contentWrapper.children(
				'div:first').css(jQueryStyle(self.options.style));
		...
		// If an explicit width is set, updateWidth prior to setting content to
		// prevent dirty rendering
		if (typeof self.options.style.width.value == 'number')
			self.updateWidth();
		...
		if ($('<canvas>').get(0).getContext || $.browser.msie) {
			...
			if (self.options.style.border.radius > 0)
				...
				self.elements.contentWrapper.css({
					border : self.options.style.border.width + 'px solid '
							+ self.options.style.border.color
				});
		...
		// Use title string for content if present
			...
			content = self.elements.target.attr('title').replace("\\n",
					'<br />');
			self.elements.target.attr('title', ''); // Remove title attribute to
		...
		// No title is present, use alt attribute instead
			...
			self.elements.target.attr('alt', ''); // Remove alt attribute to
													// prevent default tooltip
													// showing
		}
	...
	function createBorder() {
		...
		self.elements.wrapper.find('.qtip-borderBottom, .qtip-borderTop')
				.remove();
		...
		for (i in coordinates) {
			...
			containers[i] = '<div rel="' + i + '" style="'
					+ ((i.search(/Left/) !== -1) ? 'left' : 'right') + ':0; '
					+ 'position:absolute; height:' + radius + 'px; width:'
					+ radius
			...
			// No canvas, but if it's IE use VML
				...
				size = radius * 2 + 3;
				containers[i] += '<v:arc stroked="false" fillcolor="'
						+ color
						+ '" startangle="'
						+ coordinates[i][0]
						+ '" endangle="'
						...
						+ size
						+ 'px; margin-top:'
						+ ((i.search(/bottom/) !== -1) ? -2 : -1)
						+ 'px; '
						+ 'margin-left:'
						+ ((i.search(/Right/) !== -1) ? coordinates[i][2] - 3.5
								: -1)
						+ 'px; '
		...
		betweenWidth = self.getDimensions().width
				- (Math.max(width, radius) * 2);
		...
		borderTop = '<div class="qtip-borderTop" dir="ltr" style="height:'
				+ radius + 'px; ' + 'margin-left:' + radius
				+ 'px; line-height:0.1px; font-size:1px; padding:0;">'
				+ containers['topLeft'] + containers['topRight']
				+ betweenCorners;
		...
		if ($('<canvas>').get(0).getContext) {
			self.elements.wrapper.find('canvas').each(
					function() {
						borderCoord = coordinates[$(this).parent('[rel]:first')
								.attr('rel')];
						drawBorder.call(self, $(this), borderCoord, radius,
								color);
					})
		}

		// Create a phantom VML element (IE won't show the last created VML
		// element otherwise)
		else if ($.browser.msie)
			self.elements.tooltip
	...
	function createTip(corner) {
		...
		// Canvas not supported - Use VML (IE)
			...
			coordsize = self.options.style.tip.size.width + ','
					+ self.options.style.tip.size.height;
			...
			// Create VML element
			self.elements.tip += '<v:shape fillcolor="'
					+ color
					...
					+ 'vertical-align:'
					+ ((corner.search(/top/) !== -1) ? 'bottom' : 'top')
					+ '"></v:shape>';
		...
		// Create element reference and draw the canvas tip (Delayed til after
		// DOM creation)
		...
		if (corner.search(/top/) !== -1 && $.browser.msie
				&& parseInt($.browser.version.charAt(0)) === 6)
	...
	function positionTip(corner) {
		...
		if (self.options.style.tip.corner === false || !self.elements.tip)
			return;
		if (!corner)
			corner = self.elements.tip.attr('rel');
		...
		if (corner.search(/top|bottom/) !== -1) {
			...
			// Adjustments for IE6 - 0.5px border gap bug
			if ($.browser.msie)
				positionAdjust = (parseInt($.browser.version.charAt(0)) === 6) ? 1
						: ((corner.search(/left/) !== -1) ? 1 : 2);
		...
		paddingSize = self.options.style.tip.size[(paddingCorner
	...
	function createTitle() {
		...
		self.elements.title = $(
				'<div class="' + self.options.style.classes.title + '">').css(
				jQueryStyle(self.options.style.title, true)).css({
		...
		if (self.options.content.title.button !== false
			...
			self.elements.button = $(
					'<a class="' + self.options.style.classes.button
							+ '" style="float:right; position: relative"></a>')
					.css(jQueryStyle(self.options.style.button, true)).html(
							self.options.content.title.button).prependTo(
	...
	function assignEvents() {
		...
		function showMethod(event) {
			...
			if (self.options.hide.when.event == 'inactive') {
				...
				$(inactiveEvents).each(
						function() {
							hideTarget.bind(this + '.qtip-inactive',
									inactiveMethod);
	...
	function screenAdjust(position, target, tooltip) {
		...
		if (tooltip.corner == 'center')
			return target.position // TODO: 'center' corner adjustment
		...
		overflow = {
			...
			right : (adjustedPosition.left + tooltip.dimensions.width + 2 >= $.fn.qtip.cache.screen.width
					+ $.fn.qtip.cache.screen.scroll.left),
			top : (adjustedPosition.top < $.fn.qtip.cache.screen.scroll.top),
			bottom : (adjustedPosition.top + tooltip.dimensions.height + 2 >= $.fn.qtip.cache.screen.height
					+ $.fn.qtip.cache.screen.scroll.top)
		};

		// Determine new positioning properties
		adjust = {
			left : (overflow.left && (tooltip.corner.search(/right/i) != -1 || (tooltip.corner
					.search(/right/i) == -1 && !overflow.right))),
			right : (overflow.right && (tooltip.corner.search(/left/i) != -1 || (tooltip.corner
					.search(/left/i) == -1 && !overflow.left))),
		...
		if (self.options.style.tip.corner !== false) {
			...
			if (newCorner.x !== false)
				adjustedPosition.corner = adjustedPosition.corner.replace(
						/Left|Right|Middle/, newCorner.x);
			if (newCorner.y !== false)
				adjustedPosition.corner = adjustedPosition.corner.replace(
						/top|bottom/, newCorner.y);
		...
		for (i in styleObj) {
			if (sub === true && i.search(/(tip|classes)/i) !== -1)
				delete styleObj[i];
					...
					&& i.search(/(width|border|tip|title|classes|user)/i) !== -1)
				delete styleObj[i];
		}
		;
	...
	function sanitizeStyle(style) {
		...
		if (typeof style.width.max == 'string')
			style.width.max = parseInt(style.width.max.replace(/([0-9]+)/i,
					"$1"));
		if (typeof style.width.min == 'string')
			style.width.min = parseInt(style.width.min.replace(/([0-9]+)/i,
					"$1"));
	...
	function buildStyle() {
		...
		while (typeof styleExtend[0].name == 'string') {
			...
			styleExtend
					.unshift(sanitizeStyle($.fn.qtip.styles[styleExtend[0].name]));
		}
		;
		...
		if (finalStyle.tip.corner === true)
			finalStyle.tip.corner = (self.options.position.corner.tooltip === 'center') ? false
					: self.options.position.corner.tooltip;
	...
	function bgiframe() {
		...
		self.elements.bgiframe = self.elements.wrapper.prepend(html).children(
				'.qtip-bgiframe:first');
	}
	;
	...
	$(document)
			.ready(
					function() {
						// Setup library cache with window scroll and dimensions
						// of document
						$.fn.qtip.cache = {
							screen : {
								scroll : {
						...
						// Adjust positions of the tooltips on window resize or
						// scroll if enabled
						var adjustTimer;
						$(window)
								.bind(
														...
														for (i = 0; i < $.fn.qtip.interfaces.length; i++) {
															...
															if (api.status.rendered === true
																	&& (api.options.position.type !== 'static'
																			|| api.options.position.adjust.scroll
																			&& event.type === 'scroll' || api.options.position.adjust.resize
																			&& event.type === 'resize')) {
																// Queue the
																// animation so
																// positions are
