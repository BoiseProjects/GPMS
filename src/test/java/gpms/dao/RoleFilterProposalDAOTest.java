...
//		positionTitle = "Department Chair";
		UserProfile tempUser = newUserProfileDAO.findAll().get(0); //Using the first user on the list for this:
...
//		department = "Physics";
		...
		newList = (ArrayList<Proposal>) newProposalDAO.filteredProposalList(tempUser.getId().toString(), college, department, positionType, positionTitle);
		System.out.println("Returned list size is: " + newList.size());
		for(int list = 0; list < newList.size(); list++)
		{
		System.out.println("Proposal Number: " + newList.get(list).getProposalNo());
		}
