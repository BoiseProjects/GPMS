...
/**
 * This Junit test is meant to check if the signature verification method is working correctly
 * Unfortunately, the automatic proposal creation suite we have is a bit too bloated for effective testing
 * So within this class I will create only the essentials that are needed to verify the signature checking
...
public class TestSignatureChecking {
	...
	//This is all we'll need to create to test this, we need this info for any PI, CoPI that we make
	//As well as the same info for any person we add into the database, so that we can find them.
	...
	public void testOneDeanWithOnlyPI() throws UnknownHostException 
	{
		...
		pi.setFullName(profile1.getFirstName(), profile1.getLastName());
