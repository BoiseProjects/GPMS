...
public class TestDelegationAccessControl {
	...
	// HashMap<String, Multimap<String, String>> attrMap = new HashMap<String,
	...
	//
	// String proposalID = "578691e765dbb3f187eb61be";
	...
	//
	// List<ObligationResult> preObligations = new
	// ArrayList<ObligationResult>();
	// List<ObligationResult> postObligations = new
	// ArrayList<ObligationResult>();
	// List<ObligationResult> ongoingObligations = new
	// ArrayList<ObligationResult>();
	//
	...
	// List<AttributeAssignment> assignments = ((org.wso2.balana.xacml3.Advice)
	// advice)
	// .getAssignments();
	// for (AttributeAssignment assignment : assignments) {
	...
	@Test
	public void testFilteringMDP() throws UnknownHostException {
		// ac.getXACMLdecision("Faculty", "Proposal", "Create");
		...
		subjectMap.put("department", "Computer Engineering");
		// subjectMap.put("department", "Computer Science");
		...
		actionMap.put("proposal.action", "Disapprove");
		...
		contentProfile.append("<ak:fullname>");
		contentProfile.append("Computer Engineering Associate Chair");
		contentProfile.append("</ak:fullname>");
		...
		contentProfile.append("<ak:positiontitle>");
		contentProfile.append("Department Administrative Assistant");
		contentProfile.append("</ak:positiontitle>");
				...
				for (Advice advice : advices) {
					if (advice instanceof org.wso2.balana.xacml3.Advice) {
						List<AttributeAssignment> assignments = ((org.wso2.balana.xacml3.Advice) advice)
								.getAssignments();
						for (AttributeAssignment assignment : assignments) {
							System.out.println(assignment.getAttributeId()
									+ " ::: " + assignment.getContent() + "\n");
						}
					}
				}
